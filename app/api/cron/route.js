...
export async function GET() {
    ...
    const response = await fetch('https://api.qvapay.com/p2p/completed_pairs_average?coin=BANK_CUP', { headers: { 'Cache-Control': 'no-cache' }, next: { revalidate: 0 } })
    const cupHistory = await response.json()

    const response2 = await fetch('https://api.qvapay.com/p2p/completed_pairs_average?coin=BANK_MLC', { headers: { 'Cache-Control': 'no-cache' }, next: { revalidate: 0 } })
    const mlcHistory = await response2.json()

    const response3 = await fetch('https:/api.qvapay.com/p2p/completed_pairs_average?coin=CLASICA', { headers: { 'Cache-Control': 'no-cache' }, next: { revalidate: 0 } })
